...
def require_members() -> None:
    if not Member.objects.exists():
        raise CommandError("no members in the database — run sync_content first")
...
def group_cache() -> dict[str, "Group"]:
    from pages.models import Group

    return {group.slug: group for group in Group.objects.all()}
