...
def system_detail(request, slug):
    ...
    system_issues = Issue.objects.filter(system=system).select_related("member")
    return render(
        request,
...
def member_detail(request, username):
    ...
    member_issues = Issue.objects.filter(member=member).select_related("system")
    return render(
        request,
        "pages/member_detail.html",
        {
            ...
            "pull_requests": _visible_prs(member_issues),
            "open_issues": member_issues.filter(is_pull=False, state="open").order_by(
                "-created_at"
            ),
        },
    )
