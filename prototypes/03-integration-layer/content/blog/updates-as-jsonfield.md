...
That keeps the sync path simple: `updates.py` validates `updates.json` beside
`system.md`; `sync_systems` stores the list on the row. If we later need "all maintenance windows across
systems this week", promoting the field to a model is the experiment — and its cost is part of what this
prototype is meant to surface.
